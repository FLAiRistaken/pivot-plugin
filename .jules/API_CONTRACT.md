...
### AI Insights (Pro/Elite Tiers Only)

#### Overview

Pro and Elite tier users receive AI-powered insights generated by Claude Haiku in addition to rule-based insights. AI insights include confidence scores and can be rated by users for quality improvement.

**Tier Availability:**
- Free: Rule-based insights only
- Pro: Rule-based + up to 50 AI insights per day
- Elite: Rule-based + up to 200 AI insights per day

**Rate Limits (AI Generation):**
- Pro tier: Maximum 50 AI insights per day per user
- Elite tier: Maximum 200 AI insights per day per user
- When limit exceeded, endpoint returns rule-based insights only with `has_ai_insights: false`

**Response Headers (when approaching limit):**
```
X-RateLimit-Limit-AI: 50
X-RateLimit-Remaining-AI: 12
X-RateLimit-Reset-AI: 1707350400
```

#### `POST /v1/analytics/insights/{insight_id}/feedback`

Submit user feedback on AI insight quality. Only available for Pro/Elite tiers.
...
**Parameters:**
- `feedback` (string, required): Either "helpful" or "not_helpful"
...
- `403 Forbidden`: Free tier users cannot submit feedback
- `404 Not Found`: Insight ID doesn't exist
- `422 Unprocessable Entity`: Invalid feedback value

#### Examples

**Example: Pro Tier User Receives AI Insights**

```bash
curl -X GET https://api.pivot.gg/v1/analytics/servers/{server_id}/insights \
  -H "Authorization: Bearer pvt_token" \
  -H "Content-Type: application/json"
```

Response includes both rule-based and AI insights, sorted by severity.

**Example: Submit Feedback**

```bash
curl -X POST https://api.pivot.gg/v1/analytics/insights/{insight_id}/feedback \
  -H "Authorization: Bearer pvt_token" \
  -H "Content-Type: application/json" \
  -d '{"feedback": "helpful"}'
```
...
- Updated Billing and Subscription response schemas
